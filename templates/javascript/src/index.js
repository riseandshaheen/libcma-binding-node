 ...
 * On the host (unit tests / npm start) fall back to an in-memory buffer.
 */
function openLedger() {
 ...
 * `payload` is the raw input bytes (Buffer). Return true to accept.
 *
...
function handleAdvance(request, rollup) {
  try {
...
function handleInspect(request, rollup) {
  try {
    const query = request.payload.toString("utf8");
    const [route, arg] = query.split("/");

    if (route === "balance" && arg) {
      const balance = ledger.getEtherBalance(getAddress(arg));
